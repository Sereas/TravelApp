...
CREATE FUNCTION public.get_shared_trip_data(p_share_token text) RETURNS json
    ...
    AS $$
DECLARE v_trip_id UUID; v_trip JSON; v_locations JSON; v_itinerary_rows JSON;
BEGIN
  SELECT ts.trip_id INTO v_trip_id FROM trip_shares ts WHERE ts.share_token = p_share_token AND ts.is_active = true AND (ts.expires_at IS NULL OR ts.expires_at > now());
  IF v_trip_id IS NULL THEN RETURN NULL; END IF;
  SELECT json_build_object('trip_name', t.trip_name, 'start_date', t.start_date, 'end_date', t.end_date) INTO v_trip FROM trips t WHERE t.trip_id = v_trip_id;
  SELECT COALESCE(json_agg(row_to_json(loc_q)), '[]'::json) INTO v_locations FROM (SELECT l.location_id AS id, l.name, l.city, l.address, l.google_link, l.category, l.note, l.working_hours, l.requires_booking, l.latitude, l.longitude, l.google_place_id, l.useful_link, pp.photo_url AS image_url, l.user_image_url, l.user_image_crop, pp.attribution_name, pp.attribution_uri FROM locations l LEFT JOIN place_photos pp ON pp.google_place_id = l.google_place_id WHERE l.trip_id = v_trip_id ORDER BY l.created_at) loc_q;
  SELECT COALESCE(json_agg(row_to_json(itin_q)), '[]'::json) INTO v_itinerary_rows FROM (SELECT d.day_id, d.date AS day_date, d.sort_order AS day_sort_order, d.created_at AS day_created_at, d.active_option_id AS day_active_option_id, o.option_id, o.option_index, o.starting_city AS option_starting_city, o.ending_city AS option_ending_city, o.created_at AS option_created_at, ol.id AS ol_id, ol.location_id, ol.sort_order AS ol_sort_order, ol.time_period, l.name AS loc_name, l.city AS loc_city, l.address AS loc_address, l.google_link AS loc_google_link, l.category AS loc_category, l.note AS loc_note, l.working_hours AS loc_working_hours, l.requires_booking AS loc_requires_booking, pp.photo_url AS loc_photo_url, l.user_image_url AS loc_user_image_url, l.user_image_crop AS loc_user_image_crop, pp.attribution_name AS loc_attribution_name, pp.attribution_uri AS loc_attribution_uri, l.useful_link AS loc_useful_link, l.latitude AS loc_latitude, l.longitude AS loc_longitude FROM trip_days d LEFT JOIN day_options o ON o.day_id = d.day_id LEFT JOIN option_locations ol ON ol.option_id = o.option_id LEFT JOIN locations l ON l.trip_id = d.trip_id AND l.location_id = ol.location_id LEFT JOIN place_photos pp ON pp.google_place_id = l.google_place_id WHERE d.trip_id = v_trip_id ORDER BY d.sort_order, o.option_index NULLS LAST, ol.sort_order NULLS LAST) itin_q;
  RETURN json_build_object('trip', v_trip, 'locations', v_locations, 'itinerary_rows', v_itinerary_rows);
END;
$$;
...
CREATE TABLE public.place_detail_cache (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT place_detail_cache_latitude_check CHECK (((latitude IS NULL) OR ((latitude >= ('-90'::integer)::double precision) AND (latitude <= (90)::double precision)))),
    CONSTRAINT place_detail_cache_longitude_check CHECK (((longitude IS NULL) OR ((longitude >= ('-180'::integer)::double precision) AND (longitude <= (180)::double precision))))
);

ALTER TABLE public.place_detail_cache OWNER TO postgres;

COMMENT ON TABLE public.place_detail_cache IS 'Cached Google Place Details responses keyed by google_place_id. Global cache shared across all users/trips. Avoids redundant Place Details Pro API calls ($17/1000).';

COMMENT ON COLUMN public.place_detail_cache.google_types IS 'Raw Google place type tags (e.g. restaurant, museum). Stored so suggested_category can be re-derived if mapping logic changes.';

COMMENT ON COLUMN public.place_detail_cache.photo_resource_name IS 'Google Places photo resource (e.g. places/X/photos/Y). May expire; failure to fetch is handled gracefully by ensure_place_photo.';

COMMENT ON COLUMN public.place_detail_cache.cached_at IS 'Timestamp of the Google API call that populated this row. Enables future TTL-based invalidation.';
...
ALTER TABLE ONLY public.place_detail_cache
    ADD CONSTRAINT place_detail_cache_pkey PRIMARY KEY (google_place_id);
...
ALTER TABLE public.place_detail_cache ENABLE ROW LEVEL SECURITY;
...
GRANT ALL ON TABLE public.place_detail_cache TO service_role;
