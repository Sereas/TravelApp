-- Store crop metadata for CSS-based card cropping.
-- The full (uncropped) image lives in Storage; this JSONB holds the crop region
-- as percentages (0-100): { x, y, width, height }.
ALTER TABLE locations ADD COLUMN IF NOT EXISTS user_image_crop JSONB;

-- ---------------------------------------------------------------------------
-- Recreate get_itinerary_tree (single-arg, no ownership check) — unchanged
-- ---------------------------------------------------------------------------
...
RETURNS TABLE(
    ...
    loc_requires_booking text, loc_useful_link text,
...
GRANT ALL ON FUNCTION public.get_itinerary_tree(uuid) TO service_role;

-- ---------------------------------------------------------------------------
-- Recreate get_itinerary_tree (two-arg) — add loc_user_image_crop
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Recreate get_shared_trip_data — add user_image_crop to both subqueries
-- ---------------------------------------------------------------------------
