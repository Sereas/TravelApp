...
This project uses Supabase with raw SQL migrations. No Prisma/Drizzle/Alembic.

### Workflow

```bash
# Apply migration to remote DB via MCP tool (in Claude Code)
mcp__claude_ai_Supabase__apply_migration(project_id, name, query)

# MANDATORY: also create the local file (apply_migration does NOT do this)
...
```
supabase/migrations/
  20260316100000_bug_fixes_and_perf.sql
  20260317100000_reassign_day_main_option_rpc.sql
  20260331100000_drop_duplicate_index_and_set_search_path.sql
```

- Timestamp: `YYYYMMDDHHMMSS` (check existing files to pick the next one)
- Name: `snake_case`, descriptive
- Content must be **identical** to the SQL passed to `apply_migration`

### PL/pgSQL RPC migrations

Multi-table writes must be in RPCs. Use `CREATE OR REPLACE FUNCTION` with `LANGUAGE plpgsql` and `SET search_path = public`:
...
AS $$
BEGIN
  -- atomic multi-table operations here
END;
$$;
```

New read-only RPCs must be marked `STABLE`:

```sql
CREATE OR REPLACE FUNCTION public.my_read_rpc(p_arg uuid)
RETURNS TABLE(...)
LANGUAGE sql
STABLE
SET search_path = public
