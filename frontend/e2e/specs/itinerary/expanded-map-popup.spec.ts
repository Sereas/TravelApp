...
test.describe("expanded map popup — edit/delete affordances @maps-perf", () => {
   ...
   */
  test("itinerary sidebar expanded map popup has edit-note and delete-location buttons for owner", async ({
    page,
    apiClient,
  }) => {
    ...
    await expect(page.getByText("E2E MapPin Location").first()).toBeVisible({
      timeout: 10_000,
    });
